// ═══════════════════════════════════════════════════════════
// Samvadini — Complete JS (all functions, no defer needed)
// ═══════════════════════════════════════════════════════════
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // ── Button bindings (by ID, set in HTML) ──────────────────────────────────
  const bindings = [
...
function convertToSign() {
  ...
  if (!inputText) {
    ...
    startSpeechRecognition('universal', () => convertToSign()); // Callback to retry after voice input
    return;
  }
  output.innerHTML = '';
...
// ── Site Launcher — opens websites from natural language commands ─────────────
const SITE_MAP = [
  // Video & Entertainment
  ...
  // Search on YouTube specifically
...
// Open intent keywords — must have one of these + a site name
...
function detectSiteIntent(query) {
  ...
  // Check handler-based entries first (YouTube search, Google search)
  for (const site of SITE_MAP) {
    if (!site.handler) continue;
    if (site.keys.some(k => q.includes(k))) {
      return site.handler(q);
    }
  }

  // Only open plain sites if user said a verb like "open youtube"
...
function askAssistant() {
  ...
  .then(r => r.json())
  .then(data => {
    if (responseEl) responseEl.textContent = data.response || 'No response.';
  ...
  .catch(() => {
    if (responseEl) responseEl.textContent = 'Error: could not reach server.';
  });
}
...
function convertEmotionAudio() {
  ...
  fetch('/text-to-audio', { method: 'POST', body: fd })
  .then(r => r.json())
  .then(data => {
  ...
  .catch(() => {
    if (emotionEl) emotionEl.textContent = 'Error: could not reach server.';
  });
}
...
function translateSpeak() {
  ...
  .catch(() => {
    if (resultEl) resultEl.textContent = 'Error: could not reach server.';
  });
}

// ── Smart Summary (AI Analyze) ────────────────────────────────────────────────
async function smartSummarize() {
  ...
  try {
    ...
    if (data.status === 'success') {
      ...
      document.getElementById('summarySimplified').textContent = s.simplified || text;
      document.getElementById('summaryPoints').textContent     = (s.key_points || []).join(' • ');
      document.getElementById('summaryISL').textContent        = (s.isl_words  || []).join('  ·  ');
    } else {
      document.getElementById('summarySimplified').textContent = 'Failed: ' + data.message;
    }
  } catch (e) {
    document.getElementById('summarySimplified').textContent = 'Error: ' + e.message;
  }
}
...
function showCheatsheet() {
  const c = document.getElementById('cheatsheet');
...
function startSpeechRecognition(section, callback = null) {
  ...
  recognition.onresult = function (event) {
    ...
    if (section === 'universal') {
      ...
      .then(d => { 
        ...
        if (callback) callback(); // Call callback after setting value
      })
      .catch(() => { input.value = transcript; if (callback) callback(); })
      .finally(() => mic.classList.remove('glowing'));
    } else {
  ...
  recognition.onerror = () => mic.classList.remove('glowing');
  recognition.onend   = () => mic.classList.remove('glowing');
}
...
function toggleGesturePanel() {
  ...
  if (gestureEnabled) {
    ...
    if (gestureCamera) { gestureCamera.stop(); gestureCamera = null; }
  }
}

// Pinned MediaPipe version — matches CDN scripts loaded in <head>
...
function initGestureRecognition() {
  ...
  // Silently suppress all MediaPipe console noise
  ...
  try {
    handsModel = new Hands({
      locateFile: f =>
        `https://cdn.jsdelivr.net/npm/@mediapipe/hands@${MP_HANDS_VERSION}/${f}`
    });
    ...
    // Try MediaPipe Camera util first, fall back to getUserMedia
...
async function fireGestureAction(gesture) {
  ...
  // ── Step 1: fetch voice feedback from server (non-blocking, best-effort) ──
  // If the /gesture endpoint fails or audio is missing, we still run the action.
  try {
    ...
    if (res.ok) {
      const data = await res.json();
      // Build absolute URL so Audio() resolves correctly regardless of page path
      if (data.audio) {
        const audioUrl = data.audio.startsWith('http')
          ? data.audio
          : window.location.origin + (data.audio.startsWith('/') ? '' : '/') + data.audio;
        const a = new Audio(audioUrl);
        a.play().catch(() => {}); // autoplay policy — silent fail is fine
      }
      // ── Step 2: execute the mapped action ──────────────────────────────────
      switch (data.action) {
      ...
      // Server returned an error status — still run the action via local map
      _runGestureActionLocally(gesture);
    }
  } catch (e) {
    // Network error (server down, offline) — run action locally, no audio
    console.warn('[Gesture] /gesture fetch failed, running action locally:', e.message);
    _runGestureActionLocally(gesture);
  }
}

// Fallback: maps gesture → action without needing the server
...
// Hook into existing askAssistant so it logs automatically
const _origAsk = window.askAssistant;
window.askAssistant = async function () {
  const input = document.getElementById('assistantInput');
  const q     = (input?.value || '').trim();
  if (q) addToHistory('user', q);
  // call original
  if (_origAsk) await _origAsk();
  // capture the response text after a short delay
  setTimeout(() => {
    const resp = document.getElementById('assistantResponse')?.innerText || '';
    if (resp && resp !== 'Say something…') addToHistory('assistant', resp);
  }, 2000);
};
...
async function loadPhraseLibrary() {
  ...
  try {
    ...
    container.innerHTML = Object.entries(phrases).map(([cat, words]) => `
      <div>
...
function usePhrase(word) {
  ...
  if (input) {
    input.value = word;
    input.focus();
    // Trigger ISL conversion immediately
...
async function quickSpeak() {
  ...
  const btn   = document.querySelector('[onclick="quickSpeak()"]');
...
async function checkHealth() {
  try {
    ...
    if (banner) {
      ...
      // Show yellow warning if Groq not configured
